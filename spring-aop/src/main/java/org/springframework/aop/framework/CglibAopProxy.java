...
class CglibAopProxy implements AopProxy, Serializable {
			...
			for (Method method : methods) {
				...
				if (!Modifier.isStatic(mod) && !Modifier.isPrivate(mod)) { // 非静态方法，并且非私有方法
					...
					if (Modifier.isFinal(mod)) {
						if (logger.isInfoEnabled() && implementsInterface(method, ifcs)) { // 因为实现了接口，所以会打印日志，推荐使用 JDK Proxy
					...
					else if (logger.isDebugEnabled() && !Modifier.isPublic(mod) && !Modifier.isProtected(mod) && // 非公用方法，并且非保护方法
	...
	private static class CglibMethodInvocation extends ReflectiveMethodInvocation {
		...
		static boolean isMethodProxyCompatible(Method method) {
			return (Modifier.isPublic(method.getModifiers()) && // 公共方法，优化调用方式
					method.getDeclaringClass() != Object.class // 非 Object 类的方法
